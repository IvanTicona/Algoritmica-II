...
//saber si un triangulo es rectangulo

bool is_right_triangle(Point p, Point q, Point r){
    int c = dist_square(p, q);
    int b = dist_square(p, r);
    int a = dist_square(q, r);
...
int main()
{
   ...
   for(int i = 0; i < n; i++) {
    ...
    cin>>a>>b;
    v[i] = Point(a,b);
    pos[make_pair(a,b)] = i+1;
   }
   vector<Point> chull = ConvexHull(v);
